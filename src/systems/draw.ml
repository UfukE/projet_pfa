...
let string_of_building = function
  | None -> "None"
  | Some(Cst.Archer) -> "Archer"
  | Some(Cst.Bomber) -> "Bomber"
  | Some(Cst.Freezer) -> "Freezer"

let draw_ui surface ww wh bld =
  (*bottom panel ui*)
  let Global.{ctx; money; wave;_} = Global.get () in
  let panel_h = 110 in
  ...
  let fnt = Gfx.load_font "monospace" "" 16 in
  let money_s = Gfx.render_text ctx (Printf.sprintf "Money: $%d" money) fnt in
  let wave_s = Gfx.render_text ctx (Printf.sprintf "Wave: %d" wave) fnt in
  let shop_s = Gfx.render_text ctx "[1] Archer (20$)  [2] Bomber (40$)  [3] Freezer (30$)  [0] Clear" fnt in
  let start_s = Gfx.render_text ctx "[SPACE] Start next wave" fnt in
  let selected_s = Gfx.render_text ctx (Printf.sprintf "Selected: %s" (string_of_building bld)) fnt in
  ...
  Gfx.blit ctx surface selected_s 350 (wh - panel_h + 12)

let update _dt el =
  let Global.{window; ctx; money; wave; mode} = Global.get () in
  let surface = Gfx.get_surface window in
  let ww, wh = Gfx.get_context_logical_size ctx in
  ...
  match mode with
  |Build(bld) -> draw_ui surface ww wh bld
  |_ -> ()
  ;
